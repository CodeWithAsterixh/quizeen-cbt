  ...
  return (
    <Card style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <table className="data-table" aria-label="Student results for this exam">
        <thead>
          <tr>
            <th>Student Name</th><th>Class</th><th>Score</th><th>Grade</th><th>Status</th><th>Submitted</th><th style={{ textAlign: 'right' }}>Action</th>
          </tr>
        </thead>
        <tbody>
          {submissions.length === 0 ? (
            <tr><td colSpan={7} style={{ textAlign: 'center', padding: '2.5rem', color: 'var(--color-text-muted)' }}>No student submissions recorded for this test paper yet.</td></tr>
          ) : (
            submissions.map((sub) => {
              ...
              return (
                <tr key={sub.id} onClick={() => !isInProgress && onSelectSubmission(sub)} style={{ cursor: isInProgress ? 'default' : 'pointer' }}>
                  ...
                  <td>{isInProgress ? 'In Progress' : isGraded ? `${sub.score} / ${sub.totalPoints} (${pct}%)` : 'Not Graded'}</td>
                  <td>{isInProgress ? '-' : <Badge color={badgeColor} style={{ fontWeight: 800 }}>Grade {grade}</Badge>}</td>
                  <td><span className={`badge ${isInProgress ? 'badge-primary' : isGraded ? (passed ? 'badge-success' : 'badge-danger') : 'badge-warning'}`}>{isInProgress ? 'In Progress' : isGraded ? (passed ? 'Passed' : 'Failed') : 'Awaiting Result'}</span></td>
