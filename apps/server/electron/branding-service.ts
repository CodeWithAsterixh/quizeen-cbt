...
export function getInitialAppName(): string {
  try {
    ...
    if (fs.existsSync(f)) {
      ...
      if (name) return `${name} Local Server`;
    }
  } catch {}
  return bakedWhitelabelConfig?.serverName || (bakedWhitelabelConfig?.schoolName ? `${bakedWhitelabelConfig.schoolName} Local Server` : 'Queez CBT Server');
}
...
export async function applyRuntimeBranding(b: BrandingInfo, win: BrowserWindow | null): Promise<boolean> {
  ...
  const newTitle = `${brandName} Local Server`;
  app.name = newTitle;
